...
function isValidTodo(item: any): item is Schema["Todo"]["type"] {
  return item !== null && item !== undefined &&
         typeof item.id === 'string' &&
         typeof item.dateSlot === 'string' &&
...
function isValidWaitlistEntry(item: any): item is Schema["WaitlistEntry"]["type"] {
  return item !== null && item !== undefined &&
         typeof item.id === 'string' &&
         typeof item.email === 'string' &&
         (item.firstName === undefined || typeof item.firstName === 'string' || item.firstName === null) &&
         (item.lastName === undefined || typeof item.lastName === 'string' || item.lastName === null) &&
         (item.createdAt === undefined || typeof item.createdAt === 'string' || item.createdAt === null); // createdAt is datetime, which is string from API
}
...
function App() {
  ...
  const handleSlotClick = async (dateSlot: string, timeSlot: string) => {
    ...
    try {
      ...
      if (error && error.errors && Array.isArray(error.errors) && error.errors.some((e: any) => e.errorType === 'Unauthorized')) {
  ...
  const handleWaitlistToggle = async () => {
    ...
    try {
      ...
      if (error && (error as any).errors && Array.isArray((error as any).errors) && (error as any).errors.some((e: any) => e.errorType === 'Unauthorized')) {
